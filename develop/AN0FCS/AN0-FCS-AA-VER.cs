...
Target tgt = new Target();//目标
List<Target> list__tgts_of_at;//列表 自动炮台目标

HashSet<IMyCameraBlock> set_cfs;//哈希集合 camera flags
...
void update_script()
{
	...
	foreach(var i in graph_grids.list_cameras) sum_range+=i.AvailableScanRange;
	double inc = 0.5*((inc=Math.Abs((tmp=sum_range-tmp)-variation_range)/variation_range)>1 ? 1 : inc);
	variation_range=inc*variation_range+(1-inc)*tmp;//逼近新的值, tmp为当前增量, inc为增量的变化率0.5*[0,1]
	...
	foreach(var item in graph_grids.list_controllers)
		if(item.IsUnderControl) { c_main=item; break; }//找到第一个处于控制的控制器
	...
	try
	{
		...
		case ControlMode.None://无控制
			{
				if(flag__reset_turret_angle_when_user_leave&&mode__control_prev!=ControlMode.None&&!flag__a_c)
...
//扫描(搜寻目标)
bool scan(bool global = true,Vector3D? v_tgt = null)
{
	if(graph_grids.list_cameras.Count==0) return false;//检查摄像头数量
	if(v_tgt==null) v_tgt=c_main.GetPosition()+vector__global_orientation*(global ? distance__scan_under_global_control_mode : distance__scan_under_single_control_mode);
	var i = new MyDetectedEntityInfo(); float yaw, pitch; double d = 0;
	if(times__before_next_scan==0)
	{
		foreach(var c in graph_grids.list_cameras)
		{
			var _tgt = v_tgt.Value-c.WorldMatrix.Translation; global_to_yaw_pitch(out yaw,out pitch,c.WorldMatrix,_tgt);
			if(Math.Abs(yaw)>c.RaycastConeLimit||Math.Abs(pitch)>c.RaycastConeLimit) continue;
			i=c.Raycast(1.05*_tgt.Length(),pitch,yaw);
			if(graph_grids.set__self_ids.Contains(i.EntityId)) continue;
			d=(i.Position-c.WorldMatrix.Translation).Length(); break;
		}
		if(flag__enable_n_raycast) times__before_next_scan=1;
		else if(v_tgt==null) times__before_next_scan=variation_range>0 ? (global ? (long)(distance__scan_under_global_control_mode/variation_range) : (long)(distance__scan_under_single_control_mode/variation_range)) : long.MaxValue;
		else times__before_next_scan=(long)(d/variation_range);
		if(times__before_next_scan==0) times__before_next_scan=1;
	}
...
void lock_target()
{
	...
	if(!tgt.acc_avl.IsZero()) { pa+=0.5*tgt.acc*t*t;pa_avl+=0.5*tgt.acc_avl*t*t; }
	pm=(pa+pv)/2;//中点
	pm2=(p+pm)/2;//中点
...
void display_info()
{
	...
	string_builder__default_info.Append(
		...
		+$"\n<target.name|.id> {tgt.i.Name} {tgt.i.EntityId}"
		+$"\n<target.speed|.acc|.acc_avl> {tgt.v.Length().ToString("f1")} {tgt.acc.Length().ToString("f1")} {tgt.acc_avl.Length().ToString("f1")}"
		+$"\n<target.type|.distance> {tgt.i.Type} {((tgt.id!=0 ? tgt.p : position)-position).Length().ToString("f2")}"
		+"\n<indicator_rotation> "+v__r_i.X.ToString("f2")+" , "+v__r_i.Y.ToString("f2")
		...
		+"\n<count_indicators> "+graph_grids.list_indicators.Count
		+"\n");
...
void init_script()
{
	...
	flag__h_c=graph_grids.list_controllers.Count!=0;
	if(flag__h_c)
	{
		vector__global_orientation=graph_grids.list_controllers[0].WorldMatrix.Forward;
		c_f=graph_grids.list_controllers.First();
	}
	...
	if(graph_grids.list_cameras.Count>0)//计算最速扫描周期
		increment__per_frame=graph_grids.list_cameras.Count*2000.0/60/4;
...
{
	if(list_turrets.Count!=0) foreach(var item in list_turrets) if(item.b_indicator!=null)
			{ vector__global_orientation=item.b_indicator.WorldMatrix.Forward; break; }
}
...
public class Target
{
	...
	public Vector3D acc { get; private set; } public Vector3D acc_avl { get; private set; }//瞬时加速度和平均加速度
	public Vector3D p { get; private set; }//位置
	public Target(long _ts,MyDetectedEntityInfo _i) { set(_ts,_i); }//构造函数
	public Target() { }//构造函数
	public long id => i.EntityId; public Vector3D v => i.Velocity;
	public Vector3D next_tick(long ts) => p+i.Velocity*(ts/1000f);//更新位置
	public void set(long _ts,MyDetectedEntityInfo _i)//设置对象
	{
		if(i.EntityId==_i.EntityId) acc_avl=acc=(_i.Velocity-i.Velocity)*1000f/(_ts-ts);
		else acc=acc_avl=Vector3D.Zero;
		ts =_ts;i=_i;p=i.Position;
	}
}
...
public class SubTurret
{
	...
	public SubTurret(Program _program,SubTurretCoreStructure _struct_core,Turret _turret,Vector3D orientation__azimuth_main,Vector3D vec_coeff)
	{
		...
		if(struct_core.list_indicators.Count!=0)
			b__t_indicator=struct_core.list_indicators.First();
		if(struct_core.list__other_blocks.Count!=0)
			b__t_indicator=struct_core.list__other_blocks.First();
		if(struct_core.list_cameras.Count!=0&&b__t_indicator==null)
			b__t_indicator=struct_core.list_cameras.First();
		if(struct_core.list_weapons.Count!=0&&b__t_indicator==null)
			b__t_indicator=struct_core.list_weapons.First();
		if(struct_core.list_controllers.Count!=0&&b__t_indicator==null)
			b__t_indicator=struct_core.list_controllers.First();

		if(b__t_indicator==null)//找不到指示器
		{
	...
	public void update()
	{
		if(turret.flag__time_to_auto_check)
		{
			...
			if(b__t_indicator==null||!b__t_indicator.IsWorking)
				set_indicator_block();
			if(b__t_indicator==null||!b__t_indicator.IsWorking)
			{
				flag_valid=false;
	...
	{
		if(struct_core.list_cameras.Count!=0&&(b__t_indicator==null||!b__t_indicator.IsFunctional))
			b__t_indicator=struct_core.list_cameras.First();
		if(struct_core.list_weapons.Count!=0&&(b__t_indicator==null||!b__t_indicator.IsFunctional))
			b__t_indicator=struct_core.list_weapons.First();
		if(struct_core.list_controllers.Count!=0&&(b__t_indicator==null||!b__t_indicator.IsFunctional))
			b__t_indicator=struct_core.list_controllers.First();
		if(struct_core.list__other_blocks.Count!=0&&(b__t_indicator==null||!b__t_indicator.IsFunctional))
			b__t_indicator=struct_core.list__other_blocks.First();
...
public class Turret
{
	private enum TurretDrivingMode//枚举 炮塔内部驱动模式
	{
	...
	Dictionary<IMyMotorStator,Vector3> dict__stators_dead_zone = new Dictionary<IMyMotorStator,Vector3>();//映射 定子的盲区数据(最小, 最大, 平分线)
	...
	double speed__pjt_max = 400;//速率 抛射物最大速度
	...
	public Turret(Program _p,TurretCoreStructure _s,int _i)//构造函数
	{
		...
		foreach(var item in list__azimuth_stators)//读取默认角度; 设置盲区区域
		{
			...
			if(item.UpperLimitRad!=float.MaxValue&&item.LowerLimitRad!=float.MinValue&&tmp>Math.PI&&tmp<2*Math.PI)
				dict__stators_dead_zone[item]=new Vector3(item.LowerLimitRad,item.UpperLimitRad,(float)((item.UpperLimitRad+item.LowerLimitRad)/2+Math.PI));
		}
	...
	public void update()//更新炮塔
	{
		...
		//自动检查
		if(p.flag__enable_auto_check)
		{
			if(times__before_next_check==0)
			{
				flag__time_to_auto_check=true;
				times__before_next_check=p.period__auto_check;
			}
		...
		if(flag__time_to_auto_check)
		{
			...
			if(flag)
			{
				...
				foreach(var item in list_subturrets)
				{
					...
					if(item.b__t_indicator==null||!item.b__t_indicator.IsFunctional)
			...
			if(b_indicator==null&&!set_turret_main_indicator_block())
			{
		...
		if(mode_driving!=TurretDrivingMode.LocalAngleDrivingMode)
		{
			if(b_indicator!=null)
			{
				...
				{
					Vector3D vt = p.flag__h_c&&flag__inherit_vehicle_speed ? tgt.v-p.c_f.GetShipVelocities().LinearVelocity : tgt.v, va = 0.5*((p.flag__h_c&&flag__affected_by_gravity ? p.c_f.GetNaturalGravity() : Vector3D.Zero)+(b_indicator.WorldMatrix.Forward*acc_pjt)-tgt.acc_avl), vb = -vt, vc = b_indicator.GetPosition()-tgt.p;
					...
					if(flag__enable_anticipation&&(!vt.IsZero()||!va.IsZero()))
					{
						...
						if(t>0) v__tgt_c+=vt*t+0.5*tgt.acc_avl*t*t;
					}
		...
		if(p.flag__enable_auto_fire)
		{
			foreach(var i in list_subturrets)
				if(i.flag_valid)
					i.set_fire(tgt!=null&&tgt.id!=0);
		}
	}
	//设置目标实体
	public void set_target_entity(Target _tgt)
	{
		//设为全局坐标驱动模式
		mode_driving=TurretDrivingMode.GlobalCoordinateDrivingMode; tgt=_tgt;
	}
	//设置目标朝向
	...
	private bool set_turret_main_indicator_block()
	{
		if(b_indicator==null)
			foreach(var item in list_subturrets)
			{
				if(item.b__t_indicator!=null)
					b_indicator=item.b__t_indicator;
				break;
			}
		if(b_indicator==null)
			return false;
	...
	public void check()
	{
		str_b__init_info.Append($"{{{p.Runtime.MaxInstructionCount-p.Runtime.CurrentInstructionCount}, {p.Runtime.MaxCallChainDepth-p.Runtime.CurrentCallChainDepth} }}");
	}
...
public class PIDCore
{
	...
	public double cal_output(double error)
	{
		...
		if(output_last>limit_upper__output) return limit_upper__output;
		else if(output_last<limit_lower__output) return limit_lower__output;
		return output_last;
	}
