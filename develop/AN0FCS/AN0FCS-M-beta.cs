...
namespace AN0FCS_MAIN_DEV
{
    class Program : MyGridProgram
    {
        ...
        bool flag__has_controller;// 标记 包含控制器
        ...
        IMyShipController controller_first = null;// 第一个控制器
        Vector3D position;// 当前位置, 该值被视作脚本所在的实体的核心位置

        Vector3D vector__global_orientation = new Vector3D();// 向量3D 脚本全局朝向
        Vector2 vector__rotation_indicator = new Vector2();// 向量2 旋转指示器

        Target target = new Target();// 目标 用户目标

        // 列表 自动炮台目标
        Dictionary<long, Target> dict__tgts_of_at = new Dictionary<long, Target>();
		...
		**************************************************************************/
        ...
        void update_script()
        {
            ...
            // 记录上一次的控制模式
            mode__control_prev = mode_control;
            ...
            if (controller_main == null)
                mode_control = ControlMode.None;// 无人控制
            else
            {
                position = controller_main.GetPosition();// 设置为当前处于控制中的驾驶舱的位置
                ...
                float t = -vector__rotation_indicator.X;
                vector__rotation_indicator.X = vector__rotation_indicator.Y * (float)ratio__mouse_azimuth_sensitivity;
                vector__rotation_indicator.Y = t * (float)ratio__mouse_elevation_sensitivity;
                ...
                if (!flag__enable_control_plane_crossing_prevention || (angle < degree_cpcp ? (vector__rotation_indicator.Y < 0) : (angle <= 180 - degree_cpcp || (vector__rotation_indicator.Y > 0))))
            ...
            foreach (var i in global.list__auto_turrets)// 更新自动炮塔的目标
            {
                if (i.HasTarget)// 存在目标
                {
                    ...
                    if (dict__tgts_of_at.ContainsKey(e.EntityId))
                        dict__tgts_of_at[e.EntityId].set(timetamps, e, position);
                    else
                        dict__tgts_of_at[e.EntityId] = new Target(timetamps, e, position);
                }
            }

            if (TK.check_time(ref t__b_n_seq_u, period__update_targets_sequence))// 更新目标序列(重新排序)
            {
                l_sbd.Sort((x, y) => x.d.CompareTo(y.d));
                l_sbd.Sort((x, y) => x.s.CompareTo(y.s));
            }
        ...
        void automatic_control()
        {
            ...
            if (flag__a_c)
                foreach (var item in list_turrets) item.set_target_entity(target);
        }


        // 扫描
        bool scan(Vector3D vector_coordinate, Vector3D vector_orientation, double distance)
            => return scan(vector_coordinate + vector_orientation* distance);

        // 扫描 (朝指定坐标投射)
        bool scan(Vector3D vector_coordinate)
        {
            if (global.list_cameras.Count == 0)
                return false;// 检查摄像头数量, 没有摄像头直接退出

            var info = new MyDetectedEntityInfo();// 对象信息
            ...
            if (t__b_n_scan == 0)
            {
                foreach (var c in global.list_cameras)
                {
                    ...
                    info = c.Raycast(1.05 * _tgt.Length(), pitch, yaw);// 投射尝试获取目标信息

                    if (global.set__self_ids.Contains(info.EntityId))
                        continue;// 目标是自身网格, 跳过

                    distance = (info.Position - c.WorldMatrix.Translation).Length();// 计算目标距离
                    break;
                }

                if (flag__enable_unlimited_raycast)
                    t__b_n_scan = 1;// 开启了非数投射, 则每一帧都投射一次
                else if (vector_coordinate == null)
                    ...
                    t__b_n_scan = (int)(distance / variation_range);
                if (t__b_n_scan == 0) t__b_n_scan = 1;
            }
            --t__b_n_scan;

            // 检查对象类型
            if (!info.IsEmpty() && distance > distance_min)
            {
                switch (info.Type)// 类型过滤器
                {
                ...
                switch (info.Relationship)// 关系过滤器
                {
                ...
                target.set(timetamps, info, position); return true;
            }
        ...
        void display_info()
        {
            ...
            str_b__p_0.Append(
                $"\n<count_update|TS|TSP> {count_update} {timetamps} {timespan}"
                + "\n<control_mode> " + mode_control + (flag__a_c ? " LOCKED" : " NO TGT")
                + "\n<indicator_rotation> " + TK.nf(vector__rotation_indicator.X) + " , " + TK.nf(vector__rotation_indicator.Y)
            ...
            str_b__tgts.Append(Target.get_title());
            str_b__tgts.Append(target);

            foreach (var i in dict__tgts_of_at.Values)
                str_b__tgts.Append(i);

            Echo(str_b__t.ToString());
            Echo(str_b__p_0.ToString());
            Echo(str_b__p_1.ToString());
        ...
        void init_script()
        {
            ...
            flag__has_controller = global.list_controllers.Count != 0;
            if (flag__has_controller)
            {
                vector__global_orientation = global.list_controllers[0].WorldMatrix.Forward;
                controller_first = global.list_controllers.First();
            }
            ...
            public void update()// 更新炮塔
            {
                ...
                if (mode_driving != TurretDrivingMode.LocalAngleDrivingMode)
                {
                    if (b_indicator != null)
                    {
                        ...
                        {
                            Vector3D vt = p.flag__has_controller && flag__inherit_vehicle_speed ? tgt.v - p.controller_first.GetShipVelocities().LinearVelocity : tgt.v,
                                va = 0.5 * ((p.flag__has_controller && flag__affected_by_gravity ? p.controller_first.GetNaturalGravity() : Vector3D.Zero) + (b_indicator.WorldMatrix.Forward * acc_pjt) - tgt.a_a), vb = -vt, vc = b_indicator.GetPosition() - tgt.p;
