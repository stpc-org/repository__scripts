...
* ### AMCCS | 高级多联装火炮控制系统脚本 ----- ###
* ### Version 0.1.1 | by SiriusZ-BOTTLE ------ ###
...
* [1] 本脚本可自定义程度极高, 完全标准化和通用化, 可自行配置选项以支持各种不同的火炮.
...
* [9] 脚本支持二段蓄力结构的新式活塞火炮, 由于此功能较为复杂, 如果需要使用该功能请联系作者.
* 
* 
* ============================== 使用方法 ==============================
* 
...
* [1] "fire_specific_group"           脚本尝试对指定编组进行一次射击(注: 会检查边界, 但越界无提示)
* [2] "toggle_inter_group_fire_mode"  脚本切换组间射击模式
* [3] "toggle_intra_group_fire_mode"  脚本切换组内射击模式
* [4] "toggle_auto_fire_onoff"        脚本开关自动射击(自动射击相当于以固定的频率执行 fire 参数)
* 
* 
* ============================== 脚本选项 ==============================
...
*     那么脚本也会控制相应的编组进行射击, 也可将其理解为武器绑定模式.
* 
...
*      开启这个选项意味着, 当你从蓝图拉到世界里的时候, 火炮会重载一次. 这个选项默认开启.
* 
* [23] period__auto_check                   (周期, 整数) 自动检查周期
* [24] period__auto_fire                    (周期, 整数) 自动射击周期
* [25] period__update_output                (周期, 整数) 更新脚本输出周期(LCD, 编程块终端)
...
*      若你的火炮采用加特林分离或者切割机分离, 那么此时脚本不会有任何动作
...
*      
* [37] distance__warhead_savety_lock       (距离, 浮点数) 弹头安全锁距离
* [38] number__warhead_countdown_seconds   (秒数, 浮点数) 弹头倒计时秒数
* 
* 
* ============================== LCD配置 ==============================
...
* 额外消息
* [0] 这是自脚本发布以来最为重要的一次更新! 个人使用时请尽量避免使用 V0.1.0之前的老版本.
* 
* ========== V0.0.5 ==========
* 常规更新
...
//字符串 脚本版本号
readonly string str__script_version = "AMCCS V0.1.1 ";
//数组 运行时字符显示
...
bool flag__enable_charge_twice_mode = false;
//标记(全局) 是否 同步的次要活塞
...
int delay_attach = 50;
//延时 收缩(默认145帧后收缩)
int dealy__pistons_retract = 145;
...
int delay__disable_merge_blocks = 3;
//延时 次要活塞伸展
int delay__minor_pistons_extend = 1;
//延时 开启固定器
int delay__enable_merge_blocks = 70;
//延时 次要活塞收缩
int delay__minor_pistons_retract = 70;
...
void init_script_config()
{
	...
	//添加配置项
	config_script.add_line("MODE OF THE SCRIPT");
	config_script.add_config_item(nameof(mode_script),() => mode_script,x => { mode_script=(ScriptMode)x; });

	config_script.add_line("INDEXES OF CANNON AND GROUP SIZE");
	...
	config_script.add_line("TAG OF THE GROUP NAME");
	...
	config_script.add_line("FUNCTIONS SWITCH");
	config_script.add_config_item(nameof(flag__auto_activate_shell),() => flag__auto_activate_shell,x => { flag__auto_activate_shell=(bool)x; });
	...
	config_script.add_config_item(nameof(flag__reload_once_after_script_initialization),() => flag__reload_once_after_script_initialization,x => { flag__reload_once_after_script_initialization=(bool)x; });
	config_script.add_config_item(nameof(flag__enable_charge_twice_mode),() => flag__enable_charge_twice_mode,x => { flag__enable_charge_twice_mode=(bool)x; });

	config_script.add_line("EXECUTION CYCLE OF SOME FUNCTIONS");
	...
	config_script.add_line("DELAY IN EXECUTING CANNON ACTION");
	...
	config_script.add_config_item(nameof(delay__try_activate_shell),() => delay__try_activate_shell,x => { delay__try_activate_shell=(int)x; });
	...
	config_script.add_line("ABOUT SHELL ACTIVATION");
	config_script.add_config_item(nameof(distance__warhead_savety_lock),() => distance__warhead_savety_lock,x => { distance__warhead_savety_lock=(double)x; });
	config_script.add_config_item(nameof(number__warhead_countdown_seconds),() => number__warhead_countdown_seconds,x => { number__warhead_countdown_seconds=(double)x; });

	config_script.parse_custom_data();

	if(flag__enable_charge_twice_mode)
	{
		config_script.add_line("ABOUT CHARGE-TWICE CANNON");

		config_script.add_config_item(nameof(tag__cannon_minor_pistons_group),() => tag__cannon_minor_pistons_group,x => { tag__cannon_minor_pistons_group=(string)x; });
		config_script.add_config_item(nameof(tag__cannon_fixators_group),() => tag__cannon_fixators_group,x => { tag__cannon_fixators_group=(string)x; });

		config_script.add_config_item(nameof(flag__synchronized_minor_pistons),() => flag__synchronized_minor_pistons,x => { flag__synchronized_minor_pistons=(bool)x; });

		config_script.add_config_item(nameof(delay__disable_merge_blocks),() => delay__disable_merge_blocks,x => { delay__disable_merge_blocks=(int)x; });
		config_script.add_config_item(nameof(delay__minor_pistons_extend),() => delay__minor_pistons_extend,x => { delay__minor_pistons_extend=(int)x; });
		config_script.add_config_item(nameof(delay__enable_merge_blocks),() => delay__enable_merge_blocks,x => { delay__enable_merge_blocks=(int)x; });
		config_script.add_config_item(nameof(delay__minor_pistons_retract),() => delay__minor_pistons_retract,x => { delay__minor_pistons_retract=(int)x; });

		config_script.parse_custom_data();
	}

	//初始化配置
	config_script.write_to_block_custom_data();
}
...
public class PistonCannon
{
	...
	public PistonCannon(Program _program,int _index,CannonGroup _group)
	{
		...
		********************/
		...
		//二段蓄力模式
		if(program.flag__enable_charge_twice_mode)
		{
			flag__enable_charge_twice_mode=true;
			...
			********************/
			...
			if(group__minor_pistons==null)
			{
				...
				flag__enable_charge_twice_mode=false;
			}
			else
			{
				//添加到列表
		...
		{
			status_cannon=CannonStatus.NotReady;//设置为未就绪状态
			command_cannon=CannonCommand.Reload;//设置重载命令
		}
		else
	...
	private void run()
	{
		...
		if(flag__enable_charge_twice_mode)
		{
			if(count_status==program.delay__disable_merge_blocks)
				disable_fixators();//禁用固定器
			if(count_status==program.delay__minor_pistons_extend)
				minor_pistons_extend();//伸展次要活塞
			if(count_status==program.delay__enable_merge_blocks)
	...
	private bool check_cannon_status()
	{
		...
		if(piston__status_indicator.Status!=PistonStatus.Retracted&&piston__status_indicator.Status!=PistonStatus.Retracting)
			...
			return false;
		else if(flag__enable_charge_twice_mode&&!check_fixators_status())
			return false;
		return true;
	}
	...
	public string get_cannon_info()
	{
		return "<cannon> -------------------- No."+this.index_cannon
			...
			+"\n<flag_ECTM> "+this.flag__enable_charge_twice_mode
...
public class CustomDataConfig
{
	...
	//添加分割线
	public bool add_line(string str_title = "TITLE")
	{
		//检查是否已经包含此配置项目
	...
	public void write_to_block_custom_data()
	{
		...
		foreach(var pair in dict__config_items)
		{
			...
			{
				if(count!=0)
					string_builder__CD_write.Append("\n");
				string_builder__CD_write.Append($"---------- {pair.Key} ----------\n");
			}
			++count;
		}
		string_builder__CD_write.Append("\n\n");
