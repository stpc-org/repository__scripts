...
# 使用说明书 | 高级多联装火炮控制系统 | Advanced Multiple Cannon Control System
...
- [使用说明书 | 高级多联装火炮控制系统 | Advanced Multiple Cannon Control System](#使用说明书--高级多联装火炮控制系统--advanced-multiple-cannon-control-system)
  - [目录](#目录)
  - [傻瓜式手册](#傻瓜式手册)
  - [脚本功能](#脚本功能)
  - [使用方法](#使用方法)
...
6. 当脚本能够正常地识别你的火炮之后, 使用参数运行脚本就可以控制你的火炮了, 详情请查阅 "**脚本参数**" 主题
...
|                指令                |                功能说明                 | 参数  |                    备注                    |
| :--------------------------------: | :-------------------------------------: | :---: | :----------------------------------------: |
|             **`fire`**             |          脚本尝试进行一次射击           |  无   |         齐射或轮射, 默认是轮射模式         |
|       (空字符串, 什么都不写)       |          脚本尝试进行一次射击           |  无   | 齐射或轮射, 默认是轮射模式 (同 **`fire`** ) |
|     **`fire_specific_group`**      | 脚本尝试对编号为 `m` 的编组进行一次射击 |  `m`  |          会检查边界, 但越界无提示          |
...
24. **`period__auto_check`** (周期, 整数) 自动检查周期
25. **`period__auto_fire`** (周期, 整数) 自动射击周期
26. **`period__update_output`** (周期, 整数) 更新脚本输出周期 (LCD, 编程块终端)(此值忌过小, 存在性能问题)
27. **`delay_release`** (延时, 整数) 火炮释放弹性势能的时刻
    ```
    在这一个动作, 如果开启了自动激活弹头功能, 那么脚本会开始尝试获取弹头对象.
    在这一个动作, 如果开启了自动锁定垂直关节功能, 那么脚本会锁定对应的转子或铰链.
    ```
     
28. **`delay__detach_begin`** (延时, 整数) 火炮开始准备进行炮弹分离的时刻
    ```
...
29. **`delay_detach`** (延时, 整数) 火炮进行炮弹分离的时刻
    ```
...
30. **`delay__detach_end`** (延时, 整数) 火炮结束炮弹分离的时刻
    ```
...
31. **`delay__try_activate_shell`** (延时, 整数) 脚本尝试激活炮弹的时刻
    ```
...
32. **`delay__try_disconnect_shell`** (延时, 整数) 脚本尝试断开炮弹连接的时刻
    ```
    如果你的火炮支持主动断开炮弹网格连接(仅针对破限火炮), 并且在设置中开启了本脚本的此功能
    则在此时刻脚本会断开 (detach) 一个活塞的活塞头来主动切断与炮弹的网格 (子网格) 连接
    ```
     
33. **`delay__pistons_extend`** (延时, 整数) 火炮活塞伸展的时刻
    ```
    在这一个动作, 如果开启了自动开关焊接器, 那么脚本会尝试打开焊接器.
    ```
     
34. **`delay_attach`** (延时, 整数) 火炮机构附着的时刻
    ```
    在这一个动作, 如果开启了自动锁定垂直关节功能, 那么脚本会解锁对应的转子或铰链.
    注意若此步骤中释放器未能全部附着, 则再下一个时刻重新尝试直到全部附着.
    ```
     
35. **`dealy__pistons_retract`** (延时, 整数) 火炮活塞收缩(蓄力)的时刻
36. **`delay__custom_interface_timer_0`** (延时, 整数) 触发第一个用户自定义定时器的时刻
37. **`delay__custom_interface_timer_1`** (延时, 整数) 触发第二个用户自定义定时器的时刻
38. **`delay__done_loading`** (延时, 整数) 火炮装填完成的时刻
    ```
    在这一个动作, 如果开启了自动开关焊接器, 那么脚本会尝试关闭焊接器.
    ```
     
39. **`delay__first_reload`** (延时, 整数) 火炮第一次自动重载的延迟时间(此值必须为非负数)
40. **`delay_pausing`** (延时, 整数) 火炮重载过程中主活塞组伸展后的强制性暂停时间(此值必须为非负数)
     
41. **`distance__warhead_savety_lock`** (距离, 浮点数) 弹头安全锁距离
42. **`number__warhead_countdown_seconds`** (秒数, 浮点数) 弹头倒计时秒数
